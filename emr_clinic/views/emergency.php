<?php
// emergency.php - Emergency department interface for emergency staff
...
// Check if user has permission to view emergency referrals (emergency staff or admin only)
if (!in_array($_SESSION['role'], ['emergency', 'admin'])) {
...
if (isset($_POST['update_status']) && isset($_POST['referral_id'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE id = ? AND to_department = 'emergency'
    ");
...
$stmt = $pdo->prepare("
    ...
    WHERE dr.to_department = 'emergency'
    ORDER BY 
        CASE dr.status 
            WHEN 'pending' THEN 1 
...
// Get statistics
$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'emergency' AND status = 'pending'");
$pending_count = $stmt->fetch()['total'];

$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'emergency' AND status = 'in_progress'");
$in_progress_count = $stmt->fetch()['total'];

$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'emergency' AND status = 'completed' AND DATE(completed_at) = CURDATE()");
...
<head>
    ...
    <title>Emergency Department - EMR Clinic System</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">
                        <i class="fas fa-ambulance"></i> Emergency Department
                    </h1>
                    ...
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-header bg-danger text-white">
                                <h5 class="mb-0">
                                    <i class="fas fa-list"></i> Emergency Referrals
                                    <span class="badge bg-light text-dark"><?php echo count($referrals); ?> referrals</span>
                                </h5>
                            </div>
                            <div class="card-body">
                                <?php if (empty($referrals)): ?>
                                    <div class="text-center py-4">
                                        ...
                                        <h5 class="text-muted">No Emergency Referrals</h5>
                                        <p class="text-muted">No patients have been referred to the emergency department yet.</p>
                                    </div>
                                <?php else: ?>
