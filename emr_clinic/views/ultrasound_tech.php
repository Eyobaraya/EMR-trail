<?php
// ultrasound_tech.php - Ultrasound department interface for ultrasound technicians
...
// Check if user has permission to view ultrasound referrals (ultrasound tech or admin only)
if (!in_array($_SESSION['role'], ['ultrasound', 'admin'])) {
...
if (isset($_POST['update_status']) && isset($_POST['referral_id'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE id = ? AND to_department = 'ultrasound'
    ");
...
$stmt = $pdo->prepare("
    ...
    WHERE dr.to_department = 'ultrasound'
    ORDER BY 
        CASE dr.status 
            WHEN 'pending' THEN 1 
...
// Get statistics
$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'ultrasound' AND status = 'pending'");
$pending_count = $stmt->fetch()['total'];

$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'ultrasound' AND status = 'in_progress'");
$in_progress_count = $stmt->fetch()['total'];

$stmt = $pdo->query("SELECT COUNT(*) as total FROM department_referrals WHERE to_department = 'ultrasound' AND status = 'completed' AND DATE(completed_at) = CURDATE()");
...
<head>
    ...
    <title>Ultrasound Department - EMR Clinic System</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">
                        <i class="fas fa-baby"></i> Ultrasound Department
                    </h1>
                    ...
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-header">
                                <h5 class="mb-0">
                                    <i class="fas fa-list"></i> Ultrasound Referrals
                            ...
                            <div class="card-body">
                                <?php if (empty($referrals)): ?>
                                    <div class="text-center py-4">
                                        ...
                                        <h5 class="text-muted">No Ultrasound Referrals</h5>
                                        <p class="text-muted">No patients have been referred to the ultrasound department yet.</p>
                                    </div>
                                <?php else: ?>
